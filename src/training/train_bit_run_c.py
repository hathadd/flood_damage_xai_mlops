...
from src.data.transforms_bit_run_c import get_bit_run_c_eval_transforms, get_bit_run_c_train_transforms
...
@dataclass
class BITRunCDataLoadersBundle:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the official Run C BIT-style flood damage classifier."
    )
    parser.add_argument(
        "--split-metadata-path",
        "--metadata-csv",
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/focal_run_c_bit_transformer/checkpoints")
    parser.add_argument(
        "--history-path",
        type=str,
        default="outputs/focal_run_c_bit_transformer/history/training_history.csv",
    )
    parser.add_argument("--figures-dir", type=str, default="outputs/focal_run_c_bit_transformer/figures")
    ...
    parser.add_argument("--mlflow-run-name", type=str, default="focal_bit_run_c_transformer")
...
def compute_class_weights_from_dataframe(
    ...
    if missing_classes:
        raise ValueError(f"Train split is missing classes required for weighting: {missing_classes}")
...
def build_bit_run_c_dataloaders(
...
) -> BITRunCDataLoadersBundle:
    split_df = load_split_metadata(split_metadata_path)
    train_transforms = get_bit_run_c_train_transforms(image_size=image_size)
    eval_transforms = get_bit_run_c_eval_transforms(image_size=image_size)
    ...
    return BITRunCDataLoadersBundle(
...
def build_bit_run_c_mlflow_params(args: argparse.Namespace, device: torch.device) -> dict[str, str | int | float | bool]:
    return {
        "model_name": "BITTransformerRunC",
        "run_profile": "run_c_bit_transformer",
        ...
        "transform_profile": "bit_run_c_moderate",
    }
...
def fit(args: argparse.Namespace) -> None:
    ...
    print(f"Using device for BIT Run C: {device}")
    ...
    with mlflow.start_run(run_name=args.mlflow_run_name):
        ...
        mlflow.log_params(build_bit_run_c_mlflow_params(args, device))

        dataloaders = build_bit_run_c_dataloaders(
            split_metadata_path=args.split_metadata_path,
            image_size=args.image_size,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
        ...
        if args.resume_from is not None:
            ...
            best_checkpoint_path = Path(args.output_dir) / "best_bit_run_c.pt"
            print(f"Resumed BIT Run C training from checkpoint: {args.resume_from}")
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            print(
                f"BIT Run C Epoch {epoch:03d}/{args.epochs:03d} | "
            ...
            if improved:
                ...
                best_checkpoint_path = save_checkpoint(
                    ...
                    checkpoint_name="best_bit_run_c.pt",
                    epoch=epoch,
                    model=model,
                    optimizer=optimizer,
                    scheduler=scheduler,
                ...
                print(f"Saved best BIT Run C checkpoint: {best_checkpoint_path}")
            ...
            if early_stopping.should_stop:
                stopped_early = True
                print(f"Early stopping triggered for BIT Run C at epoch {epoch:03d}.")
        ...
        print(f"Best BIT Run C validation macro F1: {best_val_macro_f1:.4f}")
        if best_checkpoint_path is not None:
            print(f"Best BIT Run C checkpoint path: {best_checkpoint_path}")
        print(f"Last BIT Run C checkpoint path: {last_checkpoint_path}")
