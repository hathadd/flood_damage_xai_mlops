...
def main() -> None:
    ...
    html_output = args.html_output or f"{config['paths']['reports_output_dir']}/data_drift/data_drift_report.html"
    json_output = args.json_output or f"{config['paths']['reports_output_dir']}/data_drift/data_drift_report.json"
    ...
    if not feature_columns:
        raise ValueError("No common columns available for data drift analysis.")

    report = components["Report"](metrics=[components["DataDriftPreset"]()])
    ...
    print(f"Data drift report saved to {html_path.as_posix()}")
    print(f"Data drift JSON saved to {json_path.as_posix()}")
